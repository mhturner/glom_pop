...
for s_ind, series in enumerate(matching_series):
    ...
    # Shape = (gloms x coh x dir x time)
    ...
    for coh_ind, coh in enumerate(target_coherence):
        for dir_ind, dir in enumerate(target_direction):
            ...
            trial_averages[:, coh_ind, dir_ind, :] = np.nanmean(erm_selected, axis=1)  # each trial average: gloms x time
            erm_selected.shape
...
vfh0.savefig(os.path.join(save_directory, 'coherence_eg_fly.svg'), transparent=True)
